...
function logUmzugEvent(eventName) {
  return function(name, migration) {
    console.log(chalk.yellow(`${name} ${eventName}`));
  };
}
...
module.exports = function cmdMigrate(sequelize) {
  ...
  return umzug
    ...
    .then((result) => {
      ...
      console.log(chalk.green(doneStr));
      console.log(chalk.green('='.repeat(doneStr.length)));
    })
    .catch((err) => {
      const errorStr = `Migration ERROR`;
      console.log(chalk.red.bgWhite(errorStr));
      console.log(chalk.red.bgWhite('='.repeat(errorStr.length)));
      console.log(err);
      console.log(chalk.red.bgWhite('='.repeat(errorStr.length)));
    })
