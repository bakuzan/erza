import {TOGGLE_SIDEBAR_VISIBILITY, TOGGLE_SIDEBAR_COLLAPSE, CLOSE_SIDEBAR} from '../constants/actions'
import {getUserSettings, persistUserSettings} from '../utils/common'
import {createReducer} from './utils'

const getUserSidebar = () => {
  const settings = getUserSettings();
  if (!settings || !settings.sidebar) return { isHidden: false, isCollapsed: false };
